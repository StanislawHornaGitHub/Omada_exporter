...
switch_identity_labels = ["name", "mac", "port"]
switch_port_info = [
    "portName",
    "disable",
    "profileName",
...
class Switch(BaseDeviceMetrics):
    ...
    port_info: Info = Info(
        "switch_port",
        "Switch port information details",
        switch_identity_labels,
        registry=exporter_registry,
    )
    ...
    def __update_port_status(switch_port_metrics: list[Omada.Model.Ports.SwitchPort]):
        for port in switch_port_metrics:
            ...
            Switch.port_info.labels(**(port_labels)).info(
                Switch.get_labels(port, switch_port_info)
            )
