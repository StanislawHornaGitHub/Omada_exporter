...
class AccessPoint(BaseDeviceMetrics):
    port_rx: Gauge = Gauge(
        "access_point_port_rx_sum",
        "Sum of received bytes",
        access_point_port_identity_labels,
        registry=exporter_registry,
    )
    port_tx: Gauge = Gauge(
        "access_point_port_tx_sum",
    ...
    radio_rx_util: Gauge = Gauge(
        "radio_rx_util",
        "Percentage of receive channel bandwidth usage",
        access_point_radio_labels,
        registry=exporter_registry,
    )
    radio_tx_util: Gauge = Gauge(
        "radio_tx_util",
        "Percentage of transmit channel bandwidth usage",
        access_point_radio_labels,
        registry=exporter_registry,
    )
    ...
    radio_rx_pkts_dropped: Gauge = Gauge(
        "radio_rx_pkts_dropped_sum",
        "Sum of dropped rx packets",
        access_point_radio_labels,
        registry=exporter_registry,
    )
    radio_tx_pkts_dropped: Gauge = Gauge(
        "radio_tx_pkts_dropped_sum",
        "Sum of dropped tx packets",
        access_point_radio_labels,
        registry=exporter_registry,
    )
    radio_rx_pkts_error: Gauge = Gauge(
        "radio_rx_pkts_error_sum",
        "Sum of error rx packets",
        access_point_radio_labels,
        registry=exporter_registry,
    )
    radio_tx_pkts_error: Gauge = Gauge(
        "radio_tx_pkts_error_sum",
        "Sum of error tx packets",
        access_point_radio_labels,
        registry=exporter_registry,
    )
    radio_rx_pkts_retry: Gauge = Gauge(
        "radio_rx_pkts_retry_sum",
        "Sum of retry rx packets",
        access_point_radio_labels,
        registry=exporter_registry,
    )
    radio_tx_pkts_retry: Gauge = Gauge(
        "radio_tx_pkts_retry_sum",
        "Sum of retry tx packets",
        access_point_radio_labels,
        registry=exporter_registry,
    )
    ...
    def __update_port_status(port_metrics: list[Omada.Model.Ports.AccessPointPort]):
        for port in port_metrics:
            port_labels = AccessPoint.get_labels(
                port, access_point_port_identity_labels
            )
    ...
    @staticmethod
    def __update_radio_traffic_stats(
        radio_metrics: list[Omada.Model.Ports.AccessPointRadio],
    ):
        for device in radio_metrics:
            for field_name, value in device:
                if field_name in ["radioConfig24GHz", "radioConfig50GHz"]:
                    radio_config_labels = AccessPoint.get_labels(
                        value, access_point_radio_labels
                    )

                    AccessPoint.radio_rx_util.labels(**radio_config_labels).set(
                        value.rxUtil
                    )
                    AccessPoint.radio_tx_util.labels(**radio_config_labels).set(
                        value.txUtil
                    )
                ...
                if field_name in ["radioTraffic24GHz", "radioTraffic50GHz"]:
                    radio_traffic_labels = AccessPoint.get_labels(
                        value, access_point_radio_labels
                    )
                    ...
                    AccessPoint.radio_rx_pkts.labels(**radio_traffic_labels).set(
                        value.rxPkts
                    )
                    AccessPoint.radio_tx_pkts.labels(**radio_traffic_labels).set(
                        value.txPkts
                    )
                    AccessPoint.radio_rx_pkts_dropped.labels(
                        **radio_traffic_labels
                    ).set(value.rxDropPkts)
                    AccessPoint.radio_tx_pkts_dropped.labels(
                        **radio_traffic_labels
                    ).set(value.txDropPkts)
                    AccessPoint.radio_rx_pkts_error.labels(**radio_traffic_labels).set(
                        value.rxErrPkts
                    )
                    AccessPoint.radio_tx_pkts_error.labels(**radio_traffic_labels).set(
                        value.txErrPkts
                    )
                    AccessPoint.radio_rx_pkts_retry.labels(**radio_traffic_labels).set(
                        value.rxRetryPkts
                    )
                    AccessPoint.radio_tx_pkts_retry.labels(**radio_traffic_labels).set(
                        value.txRetryPkts
                    )
