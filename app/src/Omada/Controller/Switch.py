...
class Switch:
    ...
    __switch_info_path: str = (
        "/openapi/v1/{omadacId}/sites/{siteId}/switches/{switchMac}"
    )
    __switch_port_stats_path: str = (
        "/api/v2/sites/{siteId}/stat/switches/{switchMac}/5min"
    )
        ...
        for switch in Devices.switches:
            try:
                ...
                logger.exception(
                    e,
                    exc_info=True,
                    extra={"switch_name": switch.name, "switch_mac": switch.mac},
                )
    ...
    def get_port_info() -> list[Model.Ports.SwitchPort]:
        ...
        for switch in Devices.switches:
            try:
                switch_port_response: list[dict] = Connection.Request.get(
                    Switch.__switch_port_info_path, {"switchMac": switch.mac}
                )
            except Exception as e:
                error = True
                logger.exception(
            ...
            for port in switch_port_response:
                try:
                    ...
                    switch_port_object = Model.Ports.SwitchPort(
                        **{**switch_port_details, "switchName": switch.name}
                    )
                    switch_port_result.append(switch_port_object)
                except Exception as e:
                    error = True
                    logger.warning(
                        e,
        ...
        logger.info(
            "Found {num} switch ports".format(num=len(switch_port_result)),
            extra={"devices": [item.mac for item in Devices.switches]},
        )
        set_current_span_status(error)
    ...
    def __get_port_detail(port: dict):
        ...
        return {**port, **({k: v for k, v in port_status.items()})}
